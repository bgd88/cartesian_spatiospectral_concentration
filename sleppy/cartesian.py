...
def assemble_slepian_matrix(domain, nmax, numProc):
    assert type(
        numProc) == int, "Pretty sure you should have an integer number of processors"
    N = (2 * nmax + 1)**2
...
def assemble_slepian_matrix_block(conn, domain, index_range, nmax):
    ...
    gen1 = generate_2D_basis_functions(
        nmax, domain.extent[0], domain.extent[1])
    for ii in range(N):
        b1 = next(gen1)
        if (ii >= index_range[0]) and (ii <= index_range[1]):
            gen2 = generate_2D_basis_functions(
                nmax, domain.extent[0], domain.extent[1])
            for jj in range(N):
                b2 = next(gen2)
                if jj <= ii:  # Not necessary to calculate the others due to symmetry
                    submatrix[ii - index_range[0],
                              jj] = integrate_over_domain(domain, b1, b2, nx, ny)
    conn.send([submatrix, ])
...
def parallel_reconstruct_basis_functions(conn, index_list, sorted_eigenvecs, domain, nmax, basis_function_type):
    ...
    for i in index_list:
        ...
        if basis_function_type is 'interpolated':
            ...
            slepian_function = interpolated_slepian_basis_function(
                domain, spectral_coefs, nmax)
        elif basis_function_type is 'exact':
            # Create function using spectral coefficients
            slepian_function = slepian_basis_function(
                domain, spectral_coefs, nmax)
...
def compute_slepian_basis(domain, nmax, numProc=multiprocessing.cpu_count(), basis_function_type='exact'):
    ...
    shannon = int(np.ceil(np.pi * nmax * nmax * domain.area /
                          domain.extent[0] / domain.extent[1]))
...
class slepian_basis_function(object):
    ...
    def _set_normalization_coef(self):
        self.normalization_coef = np.sqrt(
            np.sum(self.spectral_coefs * self.spectral_coefs))

    def __call__(self, x, y):
        gen = generate_2D_basis_functions(
            self.nmax, self.extent[0], self.extent[1])
        ...
        for coef in self.spectral_coefs:
            try:
                ...
                raise Exception(
                    "Mismatch between expected length of an eigenvector and its actual length")
...
class interpolated_slepian_basis_function(slepian_basis_function):
    ...
    def __init__(self, domain, spectral_coefs, nmax, nx=100, ny=100):
        super(interpolated_slepian_basis_function, self).__init__(
            domain, spectral_coefs, nmax)
    ...
    def _create_interpolator(self, nx, ny):
        ...
        slepian_grid_values = super(
            interpolated_slepian_basis_function, self).__call__(xgrid, ygrid)
        # Create linear interpolator
        pts, vals = [p for p in zip(
            xgrid.flatten(), ygrid.flatten())], slepian_grid_values.flatten()
